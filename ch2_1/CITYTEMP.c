...
int main ( int argc, char *argv[] )
{
    ...
    while ( ! feof ( fin ))
    {
        ...
        if ( fgets ( buffer, 127+1, fin ) == NULL )
            break;
        //printf("%s",buffer);
        //printf("strlen(buffer)=%d\t%d\t%d\n",strlen(buffer), buffer[strlen(buffer)-1],buffer[strlen(buffer)]);
        /* get rid of the trailing carriage return */
        /* The strlen(buffer)th is buffer[strlen(buffer)-1],it is '\n' */
